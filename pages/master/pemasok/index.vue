<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Pemasok</h1>
        <p class="text-sm text-muted-foreground mt-0.5">Kelola data pemasok / supplier</p>
      </div>
      <UButton icon="i-heroicons-plus" size="sm" @click="openCreateModal">Tambah Pemasok</UButton>
    </div>

    <div class="mb-4 flex items-center gap-3">
      <UInput v-model="searchQuery" placeholder="Cari pemasok..." icon="i-heroicons-magnifying-glass-20-solid" class="w-72" size="sm" />
    </div>

    <UCard :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      <UTable :rows="filteredItems" :columns="columns" :loading="isLoading"
        :empty-state="{ icon: 'i-heroicons-truck', label: 'Belum ada pemasok' }" sort-mode="manual" class="w-full">
      ...
      <UCard :ui="{ divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-base font-semibold leading-6">{{ isEditing ? 'Edit Pemasok' : 'Tambah Pemasok' }}</h3>
        ...
        <UForm :state="formState" :schema="formSchema" @submit="saveItem" class="space-y-4">
          <UFormGroup label="Nama Pemasok" name="nama" required>
            <UInput v-model="formState.nama" placeholder="e.g. PT Indofood CBP" />
          </UFormGroup>
          <UFormGroup label="Kategori Merek" name="kategoriMerek">
            <UInput v-model="formState.kategoriMerek" placeholder="e.g. Makanan" />
          </UFormGroup>
          <UFormGroup label="Narahubung" name="narahubung">
            <UInput v-model="formState.narahubung" placeholder="e.g. Budi" />
          </UFormGroup>
...
const columns = [
  { key: 'nama', label: 'Nama' },
  { key: 'kategoriMerek', label: 'Kategori' },
  { key: 'narahubung', label: 'Narahubung' },
...
const formSchema = z.object({
  ...
  kategoriMerek: z.string().optional().default(''),
  narahubung: z.string().optional().default(''),
  apakahAktif: z.preprocess(toNum, z.number().min(0).max(1)),
});

interface FormData { id?: number; nama: string; kategoriMerek: string; narahubung: string; apakahAktif: number }
const defaultForm = (): FormData => ({ nama: '', kategoriMerek: '', narahubung: '', apakahAktif: 1 });
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((i: any) => i.nama?.toLowerCase().includes(q) || i.kategoriMerek?.toLowerCase().includes(q));
});

async function fetchItems() {
  isLoading.value = true;
  try {
    const res: any = await api('/api/master/pemasok');
...
async function saveItem() {
  ...
  try {
    if (isEditing.value && formState.value.id) {
      await api(`/api/master/pemasok/${formState.value.id}`, { method: 'PUT', body: formState.value });
    } else {
      await api('/api/master/pemasok', { method: 'POST', body: formState.value });
    }
    closeModal();
    await fetchItems();
    toast.add({ title: 'Berhasil', description: isEditing.value ? 'Data pemasok berhasil diubah' : 'Data pemasok berhasil ditambahkan', color: 'green' });
  } catch (err: any) {
...
async function deleteItem() {
  ...
  try {
    await api(`/api/master/pemasok/${deleteTarget.value.id}`, { method: 'DELETE' });
    isDeleteModalOpen.value = false;
    deleteTarget.value = null;
    await fetchItems();
    toast.add({ title: 'Berhasil', description: 'Data pemasok berhasil dihapus', color: 'green' });
  } catch (err: any) {
    toast.add({ title: 'Gagal', description: err.data?.statusMessage || err.message, color: 'red' });
